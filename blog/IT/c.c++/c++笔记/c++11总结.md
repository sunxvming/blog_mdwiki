...
auto/decltype
类中的：
委托构造、 成员变量初始化、类型别名、final、delete、explicit、构造函数继承
...
```
当调用f<int>(10)时，由于推导模板函数过程中可以找到一个正确的版本（ Definition #2 ），所以即时int::foo（ Definition #1）是一个语法错误，也没有关系。
...
有时我们希望从表达式的类型推断出要定义的变量类型,decltype 关键字可以用于求表达式的类型
```
int i;
double t;
struct A { double x; };
...
in some cases it makes sense to provide a move constructor even when it doesn’t make sense to provide a
